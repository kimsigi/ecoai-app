...
import { PERMISSION_ITEMS } from "./permission.model";
import { useAlert } from "@/shared/ui/component/alert";
import { request } from "react-native-permissions";

export function usePermission({ onGranted }: UsePermissionProps) {
    const {alert, confirm} = useAlert();
    const [permissions, setPermissions] = useState<PermissionItem[]>(PERMISSION_ITEMS);
    ...
    useEffect(() => {
        const checkAllGranted = permissions.every((p) => p.status === PERMISSION_STATE.GRANTED);
